...
class CorefIOBSolver(CoreferenceSolverEngine):
    ...
    def load_lang(self, lang):
        lang = lang.split("-")[0]
        self.JOINER_TOKENS = JOINER_TOKENS.get(lang, [])
        self.PREV_TOKENS = PREV_TOKENS.get(lang, [])
        self.MALE_TOKENS = MALE_TOKENS.get(lang, [])
        self.FEMALE_TOKENS = FEMALE_TOKENS.get(lang, [])
        self.INANIMATE_TOKENS = INANIMATE_TOKENS.get(lang, [])
        self.NEUTRAL_COREF_TOKENS = NEUTRAL_COREF_TOKENS.get(lang, [])
        self.MALE_COREF_TOKENS = MALE_COREF_TOKENS.get(lang, [])
        self.FEMALE_COREF_TOKENS = FEMALE_COREF_TOKENS.get(lang, [])
        self.INANIMATE_COREF_TOKENS = INANIMATE_COREF_TOKENS.get(lang, [])
        self.HUMAN_TOKENS = HUMAN_TOKENS.get(lang, [])
    ...
    def pos_tag(self, spans, lang=None):
        lang = lang or self.lang
        if isinstance(spans, str):
            spans = self.tokenizer.span_tokenize(spans, lang)
        return self.postagger.postag(spans, lang)
